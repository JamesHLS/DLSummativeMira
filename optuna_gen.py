...
def objective(trial):
    glr = trial.suggest_loguniform('glr', 1e-5, 1e-1)
    dlr = trial.suggest_loguniform('dlr', 1e-5, 1e-1)
    # d_kernal = trial.suggest_int('d_kernal', 1, 8)
    # g_kernal = trial.suggest_int('g_kernal', 1, 8)
    # Generate the model.
    model = GAN(gen_lr=glr, 
            dis_lr=dlr,
            D_ksp=(4, 2, 1), 
            G_ksp=(4, 2, 1))
